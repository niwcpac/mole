...
class TestDB(unittest.TestCase):
    ...
    def test_backup_test1(self, p_call, p_backup_db):
        ...
        with mock.patch("commands.db.service_is_running", 
                        side_effect=[False,False]) as mock_method:
            ...
            self.assertEqual(list(kwargs['env'].keys()), ["BACKUP_FLAG", 
                                                          "POPULATE_DB", 
                                                          "NEWUSERID", 
                                                          "PATH"] )
    ...
    def test_backup_test2(self, p_call, p_backup_db):
        ...
        # service_is_running("db_backup") = True
        with mock.patch("commands.db.service_is_running", 
                        side_effect=[True,False]) as mock_method:
    ...
    def test_backup_test3(self, p_call, p_backup_db):
        ...
        # service_is_running("db_backup") = True
        with mock.patch("commands.db.service_is_running", 
                        side_effect=[False,True]) as mock_method:
    ...
    def test_backup_test4(self, p_call, p_backup_db):
        ...
        # service_is_running("db_backup") = True
        with mock.patch("commands.db.service_is_running", 
                        side_effect=[True,True]) as mock_method:
    ...
    def test_load1(self, p_call, p_service_is_running,p_backup_db):
        ...
        p_service_is_running.side_effect = [(False,123),
                                             False,
                                            (False,123)] # [postgres : True, db_backup : False]
        p_backup_db.return_value = {"body": "", "status": None}
        # Mock docker services
        m_calls = mock.Mock()
    ...
    def test_load2(self, p_call, p_service_is_running,p_backup_db):
        ...
        filename = "/test_filename"
        pg_container_id = 123
        p_service_is_running.side_effect = [(False,pg_container_id),
                                             False,
                                            (False,pg_container_id)] # [postgres : True, db_backup : False]
        p_backup_db.return_value = {"body": "", "status": None}
        # Mock docker services
        m_calls = mock.Mock()
        m_calls("postgres", id=True)
        ...
        with mock.patch("os.path.isfile", return_value=True) as mock_isfile:
            ...
            print(result.stdout)
            args, kwargs = cmd_args[2]
            self.assertEqual(args[0], [ "cp", filename, f"db_backup/backups/{filename[1:]}.sql"])
            assert "env" in kwargs
            self.assertEqual(list(kwargs['env'].keys()), ["BACKUP_FLAG", 
                                                          "PATH"] )
