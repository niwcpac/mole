...
# from http://stackoverflow.com/a/23321478
# Modified slightly to include ordered dict in get_api_root_view
            ...
            def get(self, request, format=None):
                ...
                for api_view_key in api_view_urls.keys():
                    ret[api_view_key] = reverse.reverse(
                        api_view_urls[api_view_key].name, request=request, format=format
                    )
                sorted_ret = OrderedDict(
                    sorted(ret.items(), key=operator.itemgetter(0))
                )
                return response.Response(sorted_ret)
